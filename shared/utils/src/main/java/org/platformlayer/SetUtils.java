...
public class SetUtils {
    ...
    public static <T> SetCompareResults<T> setCompare(Collection<T> left, Collection<T> right) {
        ...
        setCompare(getFastCompareCollection(left), getFastCompareCollection(right), results.FunctionLeftNotRight, results.FunctionRightNotLeft, results.FunctionBoth);
        return results;
    }

    private static <T> void setCompare(Collection<T> left, Collection<T> right, Sink<T> leftNotRight, Sink<T> rightNotLeft, Sink<T> both) {
        if (left == null) {
