...
public class MinaSshConnectionWrapper implements Closeable {
    ...
    public static MinaSshConnectionWrapper build(MinaSshContext sshContext, SshConnectionInfo connectionInfo) throws SshException {
    ...
    public boolean authenticateWithPublicKey(String user, KeyPair key, TimeSpan authenticationTimeout) throws SshException {
        if (state != SshConnectionState.Connected)
            throw new IllegalStateException();

        // DelegatingServerKeyVerifier picks this up
        if (serverKeyVerifier != null) {
            sshClientSession.getMetadataMap().put(ServerKeyVerifier.class, new ServerKeyVerifierAdapter(serverKeyVerifier));
        }
        ...
        if ((sshSessionStateCode & ClientSession.AUTHED) == 0) {
            ...
            sshSessionStateCode = sshClientSession.waitFor(ClientSession.CLOSED | ClientSession.AUTHED | ClientSession.WAIT_AUTH, authenticationTimeout.getTotalMilliseconds());
        }
