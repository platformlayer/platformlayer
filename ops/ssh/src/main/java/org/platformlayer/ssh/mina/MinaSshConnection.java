...
public class MinaSshConnection extends SshConnection {
        ...
        try {
            ...
            if (!sshConnection.isAuthenticationComplete()) {
                ...
                if (isAuthenticated == false) {
                    // This happens when we upload the wrong public_key... double check if you hit this!
                    // This also happens if we're running a command that isn't valid
            ...
            if (!okay) {
                // If we fail to activate for any reason, we reset the connection so that we start clean
    ...
    @Override
    protected void sshCopyData0(InputStream fileData, long dataLength, String remoteFile, String mode) throws IOException, InterruptedException, SshException {
    ...
    @Override
    protected byte[] sshReadFile0(String remoteFile) throws IOException, InterruptedException, SshException {
        MinaSshConnectionWrapper sshConnection = ensureConnected();
