...
public class MinaSshConnection extends SshConnection {
    ...
    public static final TimeSpan DEFAULT_SSH_EXECUTE_TIMEOUT = new TimeSpan(
            "15s");

    public static final TimeSpan DEFAULT_SSH_CONNECT_TIMEOUT = new TimeSpan(
            "15s");
    public static final TimeSpan DEFAULT_SSH_KEY_EXECUTE_TIMEOUT = new TimeSpan(
            "90s");
    ...
    protected MinaSshConnectionWrapper ensureConnected() throws IOException,
            SshException {
        if (sshConnection == null) {
            getSshConnection();
        }
    ...
    protected synchronized MinaSshConnectionWrapper getSshConnection()
        ...
        SshConnectionInfo connectionInfo = new SshConnectionInfo(getHost(),
                getPort(), getUser(), getKeyPair());

        if (sshConnection != null) {
            throw new IllegalStateException(
                    "Attempt to get SSH connection when connection not previously closed");
        }

        sshConnection = MinaSshConnectionWrapper.build(getSshContext(),
                connectionInfo);
    ...
    private void activateConnection(MinaSshConnectionWrapper connection)
            throws IOException, SshException {
        activateConnection(connection, DEFAULT_SSH_CONNECT_TIMEOUT,
                DEFAULT_SSH_KEY_EXECUTE_TIMEOUT);
    }

    private void activateConnection(MinaSshConnectionWrapper sshConnection,
            TimeSpan connectTimeout, TimeSpan keyExchangeTimeout)
            throws IOException, SshException {
        boolean okay = false;
        try {
            if (!sshConnection.isConnected()) {
                ...
                if (!sshConnection.isConnected()) {
                    throw new IllegalStateException(
                            "Connection completed, but could not get connection details");
                }
            } else {
                IServerKeyVerifier myServerKeyVerifier = getServerKeyVerifier();
                myServerKeyVerifier.verifyPooled(sshConnection
                        .getServerKeyVerifier());
            }

            if (!sshConnection.isAuthenticationComplete()) {
                // Authenticate
                boolean isAuthenticated = sshConnection
                        .authenticateWithPublicKey(getUser(), getKeyPair(),
                                keyExchangeTimeout);
                ...
                if (isAuthenticated == false) {
                    ...
                    throw new SshException(
                            "Authentication failed.  Tried to connect to "
                                    + getUser()
                                    + "@"
                                    + sshConnection.getConnectionInfo()
                                            .getHost());
                } else {
    ...
    @Override
    protected ProcessExecution sshExecute0(String command, TimeSpan timeout)
        ...
        try {
            ...
            int exitCode = sshExecute(command, stdoutBinary, stderrBinary,
                    null, timeout);

            ProcessExecution processExecution = new ProcessExecution(command,
                    exitCode, stdoutBinary.toByteArray(),
    ...
    int sshExecute(String command, final OutputStream stdout,
            final OutputStream stderr, ProcessStartListener listener,
            TimeSpan timeout) throws SshException, IOException,
        ...
        try {
            ...
            try {
                sshChannel.open().await(
                        DEFAULT_SSH_CONNECT_TIMEOUT.getTotalMilliseconds());
            } catch (Exception e) {
            ...
            int flags = sshChannel.waitFor(ClientChannel.EOF
                    | ClientChannel.CLOSED, timeout.getTotalMilliseconds());
            if ((flags & ClientChannel.TIMEOUT) != 0) {
                closeAndRemoveFromPool();
                throw new SshException(
                        "Timeout while waiting for SSH task to complete.  Timeout was "
                                + timeout);
            }
            ...
            if ((flags & ClientChannel.TIMEOUT) != 0) {
                closeAndRemoveFromPool();
                throw new SshException(
                        "Timeout while waiting for exit code.  Timeout was "
                                + timeout);
            }
    ...
    @Override
    protected void sshCopyData0(InputStream fileData, long dataLength,
            String remoteFile, String mode) throws IOException,
            InterruptedException, SshException {
        int lastSlash = remoteFile.lastIndexOf('/');
        if (lastSlash == -1) {
            throw new IllegalArgumentException(
                    "Expected dest file to be absolute path: " + remoteFile);
        }
        ...
        try {
            ...
            scp.put(fileData, dataLength, filename, remoteDir, mode, timeout);
    ...
    @Override
    protected byte[] sshReadFile0(String remoteFile) throws IOException,
            InterruptedException, SshException {
        MinaSshConnectionWrapper sshConnection = ensureConnected();
        ...
        try {
            ...
            TimeSpan timeout = TimeSpan.FIVE_MINUTES;
            scp.get(remoteFile, baos, timeout);
    ...
    @Override
    public SshPortForward forwardLocalPort(InetSocketAddress remoteSocketAddress)
            throws IOException, SshException {
        MinaSshConnectionWrapper sshConnection = ensureConnected();

        final ForwardLocalPort forwardLocalPort = new ForwardLocalPort(
                sshConnection.sshClientSession, remoteSocketAddress);
