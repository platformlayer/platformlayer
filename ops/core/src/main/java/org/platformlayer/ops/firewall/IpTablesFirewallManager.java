...
public class IpTablesFirewallManager extends FirewallManager {
    ...
    // // log.warn("Have not yet hit minimum uptime of " + minimumUptime + ", exiting (probably not all rules are configured yet, so this is unsafe at this time!)");
    // return;
    // }
    //
    ...
    // records.add(FirewallRecord.pass().in().protocol(Protocol.Udp).dest(PortAddressFilter.withPortRange(67, 68)).source(PortAddressFilter.withPortRange(67, 68)));
    ...
    //
    // /*
    // * // We do allow output with keep state, but it's not marked quick. // We expect other rules to be used to avoid keeping IPF state
    // * records.add(IpfManager.parseRule("pass out proto tcp from any to any keep state")); records.add(IpfManager.parseRule("pass out proto udp from any to any keep state"));
    // */
    //
    ...
    //
    // @Override
    // protected List<FirewallRecord> rewriteRule(OpsServer server, FirewallRecord rule) throws OpsException, InterruptedException {
