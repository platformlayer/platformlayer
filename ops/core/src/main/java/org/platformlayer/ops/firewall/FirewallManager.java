...
/**
 * Defines the application of the firewall configuration to a real firewall (IPF, EC2, maybe hardware firewalls in future)
 * 
 * @author justinsb
 * 
 */
public abstract class FirewallManager {
    ...
    public void configureRules(OpsTarget target, OperationType operationType, int port, List<FirewallRecord> desired) throws OpsException {
    ...
    //
    // @SupportsStandardOperations({ StandardOperation.Configure, StandardOperation.Validate, StandardOperation.ForceConfigure })
    ...
    // operation.addWarning(this, "FirewallRuleNotFound-" + NodeUtils.sanitizeKeyName(add.buildKey()), "Firewall rule not found: " + add);
    // }
    // }
    //
    // for (FirewallRecord remove : setCompareResults.rightNotLeft) {
    ...
    // } else if (operation.isValidate()) {
    // operation.addWarning(this, "ExtraFirewallRuleFound-" + NodeUtils.sanitizeKeyName(remove.buildKey()), "Extra firewall rule found, should be removed: " + remove);
    // }
    // }
    //
    // for (FirewallRecord add : deferredAdd) {
    ...
    public void doItemOperation(OpsTarget target, OperationType operationType, FirewallEntry firewallEntry) throws Exception {
    ...
    // protected List<FirewallRecord> rewriteRules(OpsServer server, List<FirewallRecord> input) throws OpsException, InterruptedException {
    ...
    protected List<FirewallRecord> rewriteRule(OpsTarget target, FirewallRecord rule) throws OpsException, InterruptedException {
    ...
    // // }
    //
    // protected void rewriteVirtualAddresses(OpsServer server, FirewallRecord rule, List<FirewallRecord> dest, boolean processSourceFilter) throws OpsException, InterruptedException {
