...
/**
 * Creates a target out of machine image that we have to chroot into. Although the default implementations of many OpsTarget might not require overloading many methods, we do overload them so we don't
 * make assumptions about the implementation. e.g. readTextFile likely calls execute('cat {0}'), which would then get chrooted; but if it switched to use scp then this would break.
 * 
 */
