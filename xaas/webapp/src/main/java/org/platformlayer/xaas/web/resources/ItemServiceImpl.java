...
public class ItemServiceImpl implements ItemService {
    ...
    @Override
    public <T extends ItemBase> List<T> findAll(OpsAuthentication authentication, Class<T> itemClass) throws OpsException {
        ModelClass<T> modelClass = (ModelClass<T>) serviceProviderDirectory.getModelClass(itemClass);
        ...
        List<T> items;
        try {
            items = repository.findAll(modelClass, project, fetchTags, SecretProvider.withAuth(authentication), Filter.EMPTY);
    ...
    <T extends ItemBase> T ensureItem(final OpsAuthentication auth, final T item, final boolean canExist, final String uniqueTagKey) throws OpsException {
        ...
        final PlatformLayerKey modelKey = new PlatformLayerKey(null, project, modelClass.getServiceType(), modelClass.getItemType(), new ManagedItemId(id));

        item.state = ManagedItemState.CREATION_REQUESTED;

        final ServiceProvider serviceProvider = modelClass.getProvider();

        final OpsContext opsContext = buildOpsContext(modelClass.getServiceType(), auth, null);
        ...
        T created = OpsContext.runInContext(opsContext, new CheckedCallable<T, Exception>() {
            ...
            public T call() throws Exception {
                ...
                if (uniqueTagKey != null) {
                    ...
                    existing = null;
                    List<T> existingList = repository.findAll(modelClass, modelKey.getProject(), fetchTags, secretProvider, filter);
                    ...
                    try {
                        boolean fetchTags = true;
                        existing = CastUtils.checkedCast(repository.getManagedItem(modelKey, fetchTags, secretProvider), javaClass);
                    } catch (RepositoryException e) {
    ...
    @Override
    public <T extends ItemBase> T putItem(final OpsAuthentication auth, final T item, String uniqueTag) throws OpsException {
        return ensureItem(auth, item, true, uniqueTag);
    }

    protected OpsContext buildOpsContext(ServiceType serviceType, OpsAuthentication auth, PlatformLayerKey jobKey) throws OpsException {
        OpsContextBuilder opsContextBuilder = injector.getInstance(OpsContextBuilder.class);
        return opsContextBuilder.buildOpsContext(serviceType, auth, jobKey);
    }
    ...
    public void deleteItem(final OpsAuthentication auth, final PlatformLayerKey key) throws OpsException {
        ...
        final OpsContext opsContext = buildOpsContext(key.getServiceType(), auth, null);
    ...
    public <T extends ItemBase> T findItem(OpsAuthentication auth, Class<T> itemClass, String id) throws OpsException {
        ...
        T managedItem;
        try {
            managedItem = CastUtils.checkedCast(repository.getManagedItem(modelKey, fetchTags, SecretProvider.withAuth(auth)), itemClass);
        } catch (RepositoryException e) {
            throw new OpsException("Error fetching item from database", e);
        }
