...
public class JdbcManagedItemRepository implements ManagedItemRepository {
    ...
    public <T extends ItemBase> List<T> findAll(ModelClass<T> modelClass, ProjectId project, boolean fetchTags, SecretProvider secretProvider, Filter filter) throws RepositoryException {
        ...
        try {
            ...
            for (ItemEntity entity : db.listItems()) {
                T item = mapToModel(project, modelClass.getServiceType(), modelClass.getItemType(), entity, jaxbHelper, secretProvider);
    ...
    static <T extends ItemBase> T mapToModel(ProjectId project, ServiceType serviceType, ItemType itemType, ItemEntity entity, JaxbHelper jaxb, SecretProvider secretProvider)
    ...
    class DbHelper extends DbHelperBase {
        ...
        public ItemEntity findByKey(ManagedItemId managedItemId) throws SQLException {
            return queries.findByKey(getAtomValue(ServiceType.class), getAtomValue(ItemType.class), getAtomValue(ProjectId.class), managedItemId.getKey());
        }

        public List<ItemEntity> listItems() throws SQLException {
            return queries.listItems(getAtomValue(ServiceType.class), getAtomValue(ItemType.class), getAtomValue(ProjectId.class));
        }
        ...
        public List<TagEntity> listTags() throws SQLException {
            return queries.listTags(getAtomValue(ServiceType.class), getAtomValue(ItemType.class), getAtomValue(ProjectId.class));
        }
        ...
        public List<TagEntity> listTagsForItem(int itemId) throws SQLException {
            ...
            return queries.listTagsForItem(getAtomValue(ServiceType.class), getAtomValue(ItemType.class), getAtomValue(ProjectId.class), itemId);
        }
        ...
        public void updateSecret(ManagedItemId itemKey, byte[] itemSecret) throws SQLException {
            int updateCount = queries.updateSecret(itemSecret, getAtomValue(ServiceType.class), getAtomValue(ItemType.class), getAtomValue(ProjectId.class), itemKey.getKey());
            if (updateCount != 1) {
                throw new IllegalStateException("Unexpected number of rows inserted");
            }
        }
        ...
        public void updateItem(ManagedItemId itemKey, byte[] data, ManagedItemState newState) throws SQLException {
            int updateCount = queries.updateItem(data, newState.getCode(), getAtomValue(ServiceType.class), getAtomValue(ItemType.class), getAtomValue(ProjectId.class), itemKey.getKey());
            if (updateCount != 1) {
                throw new IllegalStateException("Unexpected number of rows inserted");
            }
        }
    ...
    private <T extends ItemBase> T fetchItem(DbHelper db, PlatformLayerKey plk, Class<T> modelClass, SecretProvider secretProvider, boolean fetchTags) throws SQLException, RepositoryException {
    ...
    @Override
    @JdbcTransaction
    public Tags changeTags(ModelClass<?> modelClass, ProjectId project, ManagedItemId itemKey, TagChanges changeTags) throws RepositoryException {
        DbHelper db = new DbHelper(modelClass, project);
