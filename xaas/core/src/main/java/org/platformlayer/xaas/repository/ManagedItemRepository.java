...
public interface ManagedItemRepository {
	...
	<T extends ItemBase> List<T> findAll(ModelClass<T> modelClass, ProjectId project, boolean fetchTags,
			SecretProvider secretProvider, Filter filter) throws RepositoryException;

	List<ItemBase> findRoots(ProjectId project, boolean fetchTags, SecretProvider secretProvider)
			throws RepositoryException;
