...
public class MysqlUser {
	...
	public void handler(MysqlTarget mysql) throws OpsException {
		String grant = String.format("GRANT SELECT,INSERT,UPDATE,DELETE,CREATE,DROP,ALTER ON %s.* TO %s@'%s';",
				databaseName, databaseUser, "%");
		mysql.execute(grant);

		// TODO: We create a Command.Argument.format function
		String setPassword = String.format("SET PASSWORD FOR '%s'@'%s' = PASSWORD('%s');", databaseUser, "%",
				databasePassword.plaintext());
		String setPasswordMasked = String.format("SET PASSWORD FOR '%s'@'%s' = PASSWORD('%s');", databaseUser, "%",
				Command.MASKED);
