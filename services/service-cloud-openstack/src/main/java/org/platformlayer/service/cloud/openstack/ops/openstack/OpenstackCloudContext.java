...
public class OpenstackCloudContext {
    ...
    public Server createInstance(OpenstackCloud cloud, String serverName, MachineCreationRequest request) throws OpsException {
        ...
        try {
            ...
            if (!cloudBehaviours.canUploadImages()) {
                            ...
                            if (item.getKey().equals("os_version")) {
                                if (operatingSystem != null && operatingSystem.getVersion() != null) {
                                    if (Strings.equalsIgnoreCase(operatingSystem.getVersion(), item.getValue())) {
                                        ...
                                        if (Strings.equalsIgnoreCase(operatingSystem.getVersion(), "squeeze") && Strings.equalsIgnoreCase(item.getValue(), "6")) {
                                            matchesVersion = true;
                                        } else {
                                            matchesVersion = false;
                                        }
                                    } else if (Strings.equalsIgnoreCase(operatingSystem.getDistribution(), "ubuntu")) {
                                        if (Strings.equalsIgnoreCase(operatingSystem.getVersion(), "lucid") && Strings.equalsIgnoreCase(item.getValue(), "10.04LTS")) {
                                            matchesVersion = true;
                                        } else {
                                            matchesVersion = false;
                                        }
                ...
                String imageId = imageFactory.getOrCreateImageId(cloud, ImageFactory.ImageFormat.DiskQcow2, request.recipeId);
    ...
    private Flavor getClosestInstanceType(OpenstackComputeClient computeClient, MachineCreationRequest request) throws OpenstackException {
    ...
    private static Server getInstanceInfo(OpenstackComputeClient computeClient, String serverId) throws OpenstackException {
    ...
    public static AsyncServerOperation wrapServerCreate(OpenstackComputeClient client, Server server) throws OpenstackException {
