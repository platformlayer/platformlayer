...
public class HdbDatabaseEntry extends LdapEntry {
    ...
    protected void getAdditionalProperties(Multimap<String, String> additionalProperties) {
        ...
        // additionalProperties.put("olcAccess", "to dn.subtree=\"" + getLdapRoot() + "\" attrs=userPassword,shadowLastChange by dn=\"" + OpenLdapServer.ADMIN_DN + "\" write by dn=\"" + getManagerDN()
        // + "\" write by anonymous auth by self write by * none");
        additionalProperties.put("olcAccess", "to * attrs=userPassword,shadowLastChange by dn=\"" + OpenLdapServer.ADMIN_DN + "\" write by dn=\"" + getManagerDN()
                + "\" write by anonymous auth by self write by * none");
        additionalProperties.put(
                "olcAccess",
                "to * by dn=\"" + OpenLdapServer.ADMIN_DN + "\" write by dn=\"" + getManagerDN() + "\" write by dn.subtree=\""
                        + getLdapRoot().childDN(LdapAttributes.LDAP_ATTRIBUTE_OU, LdapAttributes.LDAP_USERS_CONTAINER_OU) + "\" read by anonymous auth by * none");
    ...
    protected LdifRecord queryCurrentRecord(OpsTarget target) throws OpsException {
        ...
        // List<LdifRecord> records = OpenLdapManager.doLdapQueryChildren(openLdapInstance, openLdapInstance.getAdminDN(), ldapPassword, searchBaseDN, filter);
        //
