...
public abstract class LdapEntry {
    ...
    protected void validateRecord(LdifRecord current) {
        ...
        SetCompareResults<String> objectClassesCompare = SetUtils.setCompare(current.getObjectClasses(), desired.getObjectClasses());
        if (!objectClassesCompare.isMatch()) {
            log.info("ObjectClasses mismatch: " + objectClassesCompare);
            OpsContext.get().addWarning(this, "LdapObjectClassesDoNotMatch", "Ldap object classes do not match: " + getLdapDN());
        }

        SetCompareResults<Entry<String, String>> propertiesCompare = SetUtils.setCompare(current.getProperties().entries(), desired.getProperties().entries());
        if (!isMatch(propertiesCompare, IGNORE_PROPERTIES_IN_VALIDATE)) {
            log.info("Properties mismatch: " + propertiesCompare);
            OpsContext.get().addWarning(this, "LdapPropertiesDoNotMatch", "Ldap properties do not match: " + getLdapDN());
        }
    }
