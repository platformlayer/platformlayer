...
public class DiskImageController {
    ...
    public void handler(DiskImage image) throws OpsException, IOException {
        ...
        target.executeCommand(Command.build("chroot {0} apt-get --yes update", rootfsDir).setEnvironment(httpProxyEnv).setTimeout(TimeSpan.FIVE_MINUTES));
        ...
        if (!buildTar) {
...

            {
                ...
                target.executeCommand(Command.build("chroot {0} apt-get --yes install {1}", rootfsDir, kernelPackage).setEnvironment(httpProxyEnv).setTimeout(TimeSpan.FIFTEEN_MINUTES));
            }
        }
        ...
        if (recipe.addPackage != null) {
            for (String packageName : recipe.addPackage) {
                target.executeCommand(Command.build("chroot {0} apt-get --yes install {1}", rootfsDir, packageName).setEnvironment(httpProxyEnv).setTimeout(TimeSpan.FIFTEEN_MINUTES));
        ...
        target.executeCommand(Command.build("chroot {0} apt-get --yes upgrade", rootfsDir).setEnvironment(httpProxyEnv).setTimeout(TimeSpan.FIVE_MINUTES));
